...
const palettes = {
    light: ['#faebd7', '#dec9a8', '#d8c09e', '#dfc4ab', '#d4b896', '#dbbc9f', '#d1b898', '#c9ae8c'],
    dark: ['#2c2320', '#3a2e29', '#44342e', '#382a25', '#211a17', '#33251f', '#3d2c27', '#1a1412'],
};
...
export function BackgroundEffect() {
    ...
    useEffect(() => {
        ...
        function handleMove(e: MouseEvent) {
            ...
            rafId = requestAnimationFrame(() => {
                ...
                const deltaTime = Math.max(currentTime - lastFrameTime.current, 1) / 16.67; // Normalize to 60fps
                ...
                const xImpulse = -Math.abs(mouseVelocity.current.x) * PHYSICS_CONFIG.X_SENSITIVITY;
                const cappedXImpulse = Math.max(xImpulse, -PHYSICS_CONFIG.MAX_X_ACCELERATION);
                waveAcceleration.current += cappedXImpulse * PHYSICS_CONFIG.MOMENTUM_TRANSFER;
                ...
                stretchVelocity.current += cappedYImpulse * PHYSICS_CONFIG.MOMENTUM_TRANSFER;
    ...
    useEffect(() => {
        ...
        function drawWave(
            ...
            const amplitude =
                baseAmplitude + i * 18 + Math.sin(t / 2.5 + i) * 10 + Math.abs(stretchFactor) * (30 - i * 2); // Reduced Y-stretch effect for gentleness

            const waveSpeed = wavePhaseSpeed + i * 0.08;
            const yOffset =
                height * (baseYOffset + i * 0.12) + Math.sin(t / 3 + i) * 12 + stretchFactor * (20 - i * 1); // Reduced Y-stretch effect for gentleness
            ...
            for (let j = 0; j <= points; j++) {
                ...
                const y =
                    ...
                    Math.sin(
                        ...
                        amplitude +
                    mouseYInfluence *
                        Math.sin((x / fixedSegmentWidth) * Math.PI * (PHYSICS_CONFIG.RIPPLE_FACTOR * 0.5)) +
        ...
        function animate() {
            ...
            const returnForce = (baseSpeed - waveVelocity.current) * PHYSICS_CONFIG.X_RETURN_FORCE;
